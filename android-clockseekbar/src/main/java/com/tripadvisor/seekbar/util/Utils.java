...
public class Utils {
    ...
    public static final DateTimeFormatter SIMPLE_DATE_FORMAT_AM_PM = DateTimeFormat.forPattern("h:mm");
    public static final DateTimeFormatter SIMPLE_DATE_FORMAT_MERIDIAN = DateTimeFormat.forPattern("a");
    public static final DateTimeFormatter SIMPLE_DATE_FORMAT_HOURS = DateTimeFormat.forPattern("H:mm");

    public static boolean shouldMoveClockwise(double oldValue, double newValue) {
        double dist = abs(newValue - oldValue);
        double direction = oldValue < newValue ? 1 : -1;
        direction = dist < CircularClockSeekBar.TOTAL_DEGREES_INT / 2 ? direction : -direction;
        return direction == 1;
    }

    public static double getDistanceTo(double oldValue, double newValue) {
        double totalDegrees = CircularClockSeekBar.TOTAL_DEGREES_INT;
        boolean isClockWise = shouldMoveClockwise(oldValue, newValue);
        double dist = (newValue - oldValue) % totalDegrees;
    ...
    public static double getDelta(double oldDegrees, double newDegrees) {
